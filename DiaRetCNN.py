...
def train():
    ...
    for i in range(0, 10):
        ...
        print(offset,batch_size,len(X_train))
...
for root, dirs, files in os.walk('train_images'):
    for file in files:
        ...
        if file.endswith('.png'):
            s = file.split('.')[0]
            dfrow = df[df['id_code'] == s].index.values.astype(int)[0]
            # print(dfrow)
            labels.append(df.iloc[dfrow]['diagnosis'])
            imgpath = os.path.join(root, file)
            img = Image.open(imgpath)
            print(imgpath)
            data.append(np.array(img.getdata()).reshape((512,512,3)))
            #data.append(np.array(img.getdata()).reshape((512,)))
            print(len(data))
            i = i + 1
